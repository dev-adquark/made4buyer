...
function dateValue(value:FormDataEntryValue|null){if(!value)return null;const d=new Date(String(value));return Number.isNaN(d.getTime())?null:d}
export async function POST(req:Request){
 if(!await isAdmin())return NextResponse.json({error:"Unauthorized"},{status:401});
 const f=await req.formData();const id=String(f.get("id")||"");const title=String(f.get("title")||"").trim();const rawUrl=String(f.get("url")||"").trim();
 let url:URL;try{url=new URL(rawUrl);if(!["http:","https:"].includes(url.protocol))throw new Error()}catch{return NextResponse.json({error:"Valid http(s) URL required"},{status:422})}
 if(!title)return NextResponse.json({error:"Title required"},{status:422});
 const startAt=dateValue(f.get("startAt"));const endAt=dateValue(f.get("endAt"));
 ...
 const data={title,label:String(f.get("label")||"Sponsored").trim()||"Sponsored",url:url.toString(),active:f.get("active")==="on",minEvents:Math.floor(minEvents),minSessions:Math.floor(minSessions),startAt,endAt};
