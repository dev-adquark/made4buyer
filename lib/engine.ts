...
function validReview(item:{title:string;summary:string;body:string;productName:string;confidence:number}){
  return item.title.length>=8 && item.body.length>=120 && item.productName.length>=2 && item.summary.length>=20 && Number.isFinite(item.confidence);
}
export async function runIngestion(){
  const run=await db.ingestionRun.create({data:{source:process.env.CONTENT_API_URL||"unconfigured",status:"RUNNING"}});
  let accepted=0,rejected=0,duplicate=0;
  try{
    ...
    for(const item of items){
      ...
      try{
        ...
        for(const d of deals.slice(0,5)){
          if(await verifyLink(d.url)) await db.deal.create({data:{reviewId:review.id,...d,verified:true,lastChecked:new Date()}});
        }
      }catch(error){
        await db.ingestionRun.update({where:{id:run.id},data:{errors:{dealLookup:String(error)}}});
      }
      accepted++;
    }
    await db.ingestionRun.update({where:{id:run.id},data:{finishedAt:new Date(),status:"COMPLETED",total:items.length,accepted,rejected,duplicate}});
    return {total:items.length,accepted,rejected,duplicate};
  }catch(error){
...
export async function revalidateDeals(){
  const deals=await db.deal.findMany({select:{id:true,url:true}});
  let valid=0;
  for(const d of deals){const ok=await verifyLink(d.url);await db.deal.update({where:{id:d.id},data:{verified:ok,lastChecked:new Date()}});if(ok)valid++}
  return {checked:deals.length,valid};
}