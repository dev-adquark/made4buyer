...
export default function AnalyticsTracker({reviewId,category,event="page_view"}:{reviewId?:string;category?:string;event?:"page_view"|"review_view"}){useEffect(()=>{const id=sessionId();fetch("/api/events",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({reviewId,category,event,sessionId:id})}).catch(()=>{});},[reviewId,category,event]);return null}
